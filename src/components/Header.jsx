  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'py-2 bg-[#0D1F18]/95 backdrop-blur-md shadow-lg'
          : 'py-4 bg-[#0D1F18]'
      }`}
    >
      <nav className="flex justify-between items-center w-full px-6 py-2 max-w-7xl mx-auto">
        <a href="#hero" className="flex items-center gap-2" aria-label={t.header.logoLabel}>
          <img alt="" className="h-8 w-8" src={logoSrc} />
        ...
        <div className="hidden md:flex items-center space-x-8">
          {t.header.nav.map((item) => (
            <a
              key={item.label}
              href={item.href}
              className="text-white/80 hover:text-primary-fixed text-label-md transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-primary-fixed"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            href="#contacto"
            className="hidden md:inline-flex bg-primary-fixed text-on-primary-fixed px-6 py-2.5 rounded-full font-bold text-label-md hover:brightness-110 active:scale-95 transition-all duration-200"
          >
            {t.header.cta}
          </a>
      ...
      {mobileOpen && (
        <div
          ...
          aria-modal="true"
          aria-label={t.header.menuLabel}
          className="md:hidden bg-[#0D1F18] border-t border-white/10 px-6 pb-6 pt-4 space-y-4"
          onKeyDown={(e) => e.key === 'Escape' && setMobileOpen(false)}
        >
          {t.header.nav.map((item) => (
            <a
